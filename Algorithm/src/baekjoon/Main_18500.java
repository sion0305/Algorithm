...
class Solve {
	...
	private void setCluster(boolean[][] check) {
		...
		while (!q.isEmpty()) {
			...
			q.poll();
			if (check[nowY][nowX] == true)
				continue;
			check[nowY][nowX] = true;
			for (int i = 0; i < 4; i++) {
				int nextY = nowY + dy[i];
				int nextX = nowX + dx[i];
				if (nextY < 1 || nextY > R || nextX < 1 || nextX > C || check[nextY][nextX] == true
						|| mineral[nextY][nextX] == '.')
					continue;
	...
	private boolean checkCluster(boolean[][] check) {
		for (int i = 1; i <= R; i++) {
			for (int j = 1; j <= C; j++) {
				if (check[i][j] == false && mineral[i][j] == 'x')
					return false;
			}
		}
		return true;
	...
	private boolean mergeCluster(boolean[][] check, boolean[][] mergeCheck, int y, int x) {
		...
		for (int i = 0; i < 4; i++) {
			int _y = y + dy[i];
			int _x = x + dx[i];
			if (_y > R || _y < 1 || _x > C || _x < 1 || mergeCheck[_y][_x] == true || check[_y][_x] == true
					|| mineral[_y][_x] == '.')
				continue;
